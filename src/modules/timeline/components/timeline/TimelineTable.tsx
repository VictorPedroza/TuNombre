        ...
        <div className="overflow-hidden rounded-xl border border-white/10 bg-[#111111]">
            ...
            {!loading && events.map((event) => (
                ...
                >
                    <div className="flex items-start gap-4">
                        ...
                        <div className="min-w-0 flex-1 pt-1 sm:hidden">
                            <p className="truncate text-[15px] font-semibold text-white">
                                {event.image && <span className="mr-1.5">{event.emoji}</span>}
                                {event.title}
                            </p>
                    ...
                    <div className="flex shrink-0 gap-2 sm:ml-auto sm:self-start">
                       <Button label="Editar" onClick={() => alert("Editando...")} variant="outline" />
                        <button
                            aria-label="Apagar"
                            className="flex-1 rounded-md border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-xs font-medium text-red-300 transition-colors hover:bg-red-500/20 disabled:opacity-50 sm:flex-none"
                        >
                            Apagar
                        </button>
                    </div>
